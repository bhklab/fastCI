...
naiveRCI <- function(x, y, delta_x = 0.2, delta_y = 0.2, count_xties=0, count_yties=0, 
  ...
  rcimat <- sign(xdeltamat) * (abs(xdeltamat) > delta_x) * sign(ydeltamat) * (abs(ydeltamat) > delta_y)
